...
with dpg.window(tag="Primary Window"):
    ...
    dpg.add_input_text(label="Download Source", source="downloadurl")
    dpg.add_text(" ")
    dpg.add_text("Game Settings")
    dpg.add_input_text(label="Directory", source="directory")
    
    dpg.add_input_text(label="Username", source="username")
    ...
    dpg.add_input_text(label="IP Address", source="ipaddr")
...
#    dpg.add_button(label="Launch Minecraft: Legacy Console Edition", callback=singleplayer)
    dpg.add_button(label="Launch Singleplayer", callback=singleplayer)
    dpg.add_button(label="Connect to server", callback=multiplayer)
    dpg.add_text(" ")
    dpg.add_text("Status:")
    dpg.add_text(source="status")

dpg.create_viewport(title="Legacy Console Edition Launcher - @uncreativeCultist - v1.0.1", width=784, height=441)
dpg.setup_dearpygui()
dpg.show_viewport()
